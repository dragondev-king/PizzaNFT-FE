...
export const NftTokenID = () => (dispatch, getState) => {
  try {
    axios
      .get(
        `https://deep-index.moralis.io/api/v2/nft/${process.env.REACT_APP_NFT_ADDRESS}?chain=bsc%20mainnet&format=decimal`,
        {
          headers: {
...
export const topOwner = () => (dispatch, getState) => {
  try {
    axios
      .get(
        `https://deep-index.moralis.io/api/v2/nft/${process.env.REACT_APP_NFT_ADDRESS}/owners?chain=bsc%20mainnet&format=decimal`,
        {
          headers: {
            accept: "application/json",
            "X-API-Key": process.env.REACT_APP_MORALIS_KEY,
